...
std::shared_ptr<NetworkSecurityStub> CreateDefaultNetworkSecurityStub(
  ...
  auto service_grpc_stub =
      google::cloud::networksecurity::v1::NetworkSecurity::NewStub(channel);
  ...
  std::shared_ptr<NetworkSecurityStub> stub =
      std::make_shared<DefaultNetworkSecurityStub>(
  ...
  if (auth->RequiresConfigureContext()) {
    stub =
        std::make_shared<NetworkSecurityAuth>(std::move(auth), std::move(stub));
  }
  stub = std::make_shared<NetworkSecurityMetadata>(
      std::move(stub), std::multimap<std::string, std::string>{});
  if (internal::Contains(options.get<LoggingComponentsOption>(), "rpc")) {
    GCP_LOG(INFO) << "Enabled logging for gRPC calls";
    stub = std::make_shared<NetworkSecurityLogging>(
        std::move(stub), options.get<GrpcTracingOptionsOption>(),
        options.get<LoggingComponentsOption>());
  }
  if (internal::TracingEnabled(options)) {
    stub = MakeNetworkSecurityTracingStub(std::move(stub));
  }
  return stub;
}
