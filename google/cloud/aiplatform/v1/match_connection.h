...
/// The retry policy for `MatchServiceConnection`.
class MatchServiceRetryPolicy : public ::google::cloud::RetryPolicy {
 public:
  /// Creates a new instance of the policy, reset to the initial state.
  virtual std::unique_ptr<MatchServiceRetryPolicy> clone() const = 0;
};

/**
 * A retry policy for `MatchServiceConnection` based on counting errors.
 *
...
class MatchServiceLimitedErrorCountRetryPolicy
 ...
 public:
  ...
  std::unique_ptr<MatchServiceRetryPolicy> clone() const override {
    return std::make_unique<MatchServiceLimitedErrorCountRetryPolicy>(
        maximum_failures());
  }
...
/**
 * A retry policy for `MatchServiceConnection` based on elapsed time.
 *
 * This policy stops retrying if:
 * - An RPC returns a non-transient error.
 ...
 * - [`kUnavailable`](@ref google::cloud::StatusCode)
 */
class MatchServiceLimitedTimeRetryPolicy : public MatchServiceRetryPolicy {
 public:
  /**
  ...
  std::unique_ptr<MatchServiceRetryPolicy> clone() const override {
    return std::make_unique<MatchServiceLimitedTimeRetryPolicy>(
        maximum_duration());
  }
