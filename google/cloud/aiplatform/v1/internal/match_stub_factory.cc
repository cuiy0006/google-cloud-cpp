...
std::shared_ptr<MatchServiceStub> CreateDefaultMatchServiceStub(
  ...
  auto service_grpc_stub =
      google::cloud::aiplatform::v1::MatchService::NewStub(channel);
  ...
  std::shared_ptr<MatchServiceStub> stub =
      std::make_shared<DefaultMatchServiceStub>(
          std::move(service_grpc_stub), std::move(service_iampolicy_stub),
          std::move(service_locations_stub));

  if (auth->RequiresConfigureContext()) {
    stub = std::make_shared<MatchServiceAuth>(std::move(auth), std::move(stub));
  }
  stub = std::make_shared<MatchServiceMetadata>(
      std::move(stub), std::multimap<std::string, std::string>{});
  if (internal::Contains(options.get<LoggingComponentsOption>(), "rpc")) {
    GCP_LOG(INFO) << "Enabled logging for gRPC calls";
    stub = std::make_shared<MatchServiceLogging>(
        std::move(stub), options.get<GrpcTracingOptionsOption>(),
        options.get<LoggingComponentsOption>());
  }
  if (internal::TracingEnabled(options)) {
    stub = MakeMatchServiceTracingStub(std::move(stub));
  }
  return stub;
}
