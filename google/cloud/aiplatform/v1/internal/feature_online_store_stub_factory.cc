...
#include "google/cloud/aiplatform/v1/internal/feature_online_store_metadata_decorator.h"
...
CreateDefaultFeatureOnlineStoreServiceStub(
  ...
  auto service_grpc_stub =
      google::cloud::aiplatform::v1::FeatureOnlineStoreService::NewStub(
          channel);
  ...
  std::shared_ptr<FeatureOnlineStoreServiceStub> stub =
      std::make_shared<DefaultFeatureOnlineStoreServiceStub>(
          std::move(service_grpc_stub), std::move(service_iampolicy_stub),
          std::move(service_locations_stub));

  if (auth->RequiresConfigureContext()) {
    stub = std::make_shared<FeatureOnlineStoreServiceAuth>(std::move(auth),
                                                           std::move(stub));
  }
  stub = std::make_shared<FeatureOnlineStoreServiceMetadata>(
      std::move(stub), std::multimap<std::string, std::string>{});
  if (internal::Contains(options.get<LoggingComponentsOption>(), "rpc")) {
    GCP_LOG(INFO) << "Enabled logging for gRPC calls";
    stub = std::make_shared<FeatureOnlineStoreServiceLogging>(
        std::move(stub), options.get<GrpcTracingOptionsOption>(),
        options.get<LoggingComponentsOption>());
  }
  if (internal::TracingEnabled(options)) {
    stub = MakeFeatureOnlineStoreServiceTracingStub(std::move(stub));
  }
  return stub;
}
