...
std::shared_ptr<DataScanServiceStub> CreateDefaultDataScanServiceStub(
  ...
  auto service_grpc_stub =
      google::cloud::dataplex::v1::DataScanService::NewStub(channel);
  ...
  std::shared_ptr<DataScanServiceStub> stub =
      std::make_shared<DefaultDataScanServiceStub>(
  ...
  if (auth->RequiresConfigureContext()) {
    stub =
        std::make_shared<DataScanServiceAuth>(std::move(auth), std::move(stub));
  }
  stub = std::make_shared<DataScanServiceMetadata>(
      std::move(stub), std::multimap<std::string, std::string>{});
  if (internal::Contains(options.get<LoggingComponentsOption>(), "rpc")) {
    GCP_LOG(INFO) << "Enabled logging for gRPC calls";
    stub = std::make_shared<DataScanServiceLogging>(
        std::move(stub), options.get<GrpcTracingOptionsOption>(),
        options.get<LoggingComponentsOption>());
  }
  if (internal::TracingEnabled(options)) {
    stub = MakeDataScanServiceTracingStub(std::move(stub));
  }
  return stub;
}
