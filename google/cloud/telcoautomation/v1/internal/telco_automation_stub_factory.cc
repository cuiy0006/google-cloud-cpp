...
std::shared_ptr<TelcoAutomationStub> CreateDefaultTelcoAutomationStub(
    std::shared_ptr<internal::GrpcAuthenticationStrategy> auth,
    Options const& options) {
  auto channel = auth->CreateChannel(options.get<EndpointOption>(),
                                     internal::MakeChannelArguments(options));
  auto service_grpc_stub =
      google::cloud::telcoautomation::v1::TelcoAutomation::NewStub(channel);
  auto service_locations_stub =
      google::cloud::location::Locations::NewStub(channel);
  std::shared_ptr<TelcoAutomationStub> stub =
      std::make_shared<DefaultTelcoAutomationStub>(
          std::move(service_grpc_stub), std::move(service_locations_stub),
          google::longrunning::Operations::NewStub(channel));

  if (auth->RequiresConfigureContext()) {
    stub =
        std::make_shared<TelcoAutomationAuth>(std::move(auth), std::move(stub));
  }
  stub = std::make_shared<TelcoAutomationMetadata>(
      std::move(stub), std::multimap<std::string, std::string>{});
  if (internal::Contains(options.get<LoggingComponentsOption>(), "rpc")) {
    GCP_LOG(INFO) << "Enabled logging for gRPC calls";
    stub = std::make_shared<TelcoAutomationLogging>(
        std::move(stub), options.get<GrpcTracingOptionsOption>(),
        options.get<LoggingComponentsOption>());
  }
  if (internal::TracingEnabled(options)) {
    stub = MakeTelcoAutomationTracingStub(std::move(stub));
  }
  return stub;
}
