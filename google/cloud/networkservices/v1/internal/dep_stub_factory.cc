...
std::shared_ptr<DepServiceStub> CreateDefaultDepServiceStub(
  ...
  auto service_grpc_stub =
      google::cloud::networkservices::v1::DepService::NewStub(channel);
  ...
  std::shared_ptr<DepServiceStub> stub =
      std::make_shared<DefaultDepServiceStub>(
          std::move(service_grpc_stub), std::move(service_iampolicy_stub),
          std::move(service_locations_stub),
          google::longrunning::Operations::NewStub(channel));

  if (auth->RequiresConfigureContext()) {
    stub = std::make_shared<DepServiceAuth>(std::move(auth), std::move(stub));
  }
  stub = std::make_shared<DepServiceMetadata>(
      std::move(stub), std::multimap<std::string, std::string>{});
  if (internal::Contains(options.get<LoggingComponentsOption>(), "rpc")) {
    GCP_LOG(INFO) << "Enabled logging for gRPC calls";
    stub = std::make_shared<DepServiceLogging>(
        std::move(stub), options.get<GrpcTracingOptionsOption>(),
        options.get<LoggingComponentsOption>());
  }
  if (internal::TracingEnabled(options)) {
    stub = MakeDepServiceTracingStub(std::move(stub));
  }
  return stub;
}
