...
std::shared_ptr<FunctionServiceStub> CreateDefaultFunctionServiceStub(
  ...
  auto service_grpc_stub =
      google::cloud::functions::v2::FunctionService::NewStub(channel);
  ...
  std::shared_ptr<FunctionServiceStub> stub =
      std::make_shared<DefaultFunctionServiceStub>(
  ...
  if (auth->RequiresConfigureContext()) {
    stub =
        std::make_shared<FunctionServiceAuth>(std::move(auth), std::move(stub));
  }
  stub = std::make_shared<FunctionServiceMetadata>(
      std::move(stub), std::multimap<std::string, std::string>{});
  if (internal::Contains(options.get<LoggingComponentsOption>(), "rpc")) {
    GCP_LOG(INFO) << "Enabled logging for gRPC calls";
    stub = std::make_shared<FunctionServiceLogging>(
        std::move(stub), options.get<GrpcTracingOptionsOption>(),
        options.get<LoggingComponentsOption>());
  }
  if (internal::TracingEnabled(options)) {
    stub = MakeFunctionServiceTracingStub(std::move(stub));
  }
  return stub;
}
